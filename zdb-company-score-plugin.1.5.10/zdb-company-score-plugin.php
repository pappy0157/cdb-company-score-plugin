...
class CDB_Company_Score_Plugin {
                ...
                <div class="cdb-field">
                    <label>法人区分
                        <select name="<?php echo esc_attr($m['corp_class']); ?>">
                            <?php $opts=['listed'=>'上場','governmentmunicipality'=>'政府・官公庁・地方自治体','university'=>'大学・学校法人','large'=>'大企業','mid'=>'中堅','sme'=>'中小','sole'=>'個人事業'];foreach($opts as $k=>$lbl){$sel=selected($vals['corp_class'],$k,false);echo "<option value='$k' $sel>$lbl</option>";}?>
                        </select>
                    </label>
                </div>
                <div class="cdb-field">
    ...
    private function score_corp_class($cls, $max){
        switch ($cls){
            ...
            case 'governmentmunicipality': return $max;        // 100%
            case 'university': return $max * 0.85;  // 85%（学校法人・国公私立大学・各種学校）
    ...
    private function label_corp_class($cls){
        switch ($cls){
            ...
            case 'governmentmunicipality': return '政府・官公庁・地方自治体';
            case 'university':           return '大学・学校法人';
    ...
    private function diagnostic_text($section, $pct, $sc){
        ...
        switch($section){
            ...
            case 'credit':
                ...
                $detail = '<ul class="dx-list">'
                    .'<li>法人区分：'.esc_html($cls_label).'(※<a href="https://companydata.tsujigawa.com/definition-of-company-size/" target="_blank">定義について</a>)</li>'
                    .'<li>設立：'.esc_html($ey).'（創業'.esc_html($age).'）</li>'
                    .'<li>資本金：'.esc_html($cap).'</li>'
                    .'<li>補助金・官公庁採択：'.esc_html($gr).'</li>'
                    .'</ul>';
    ...
    <div class="cdb-r-wrap" data-endpoint="<?php echo esc_url($rest); ?>" data-nonce="<?php echo esc_attr($nonce); ?>" data-pt="<?php echo esc_attr($atts['post_type']); ?>">
      ...
      <div class="cdb-r-controls" id="cdbRCtl">
        ...
        <select id="rcorp">
          ...
          <option value="listed">上場</option><option value="governmentmunicipality">政府・官公庁・地方自治体</option><option value="university">大学・学校法人</option><option value="large">大企業</option>
