...
add_shortcode('cdb_company_ranking_rest', function($atts){
    ...
    <div class="cdb-r-wrap" data-endpoint="<?php echo esc_url($rest); ?>
      <span id="cdbRTop" aria-hidden="true"></span>" data-nonce="<?php echo esc_attr($nonce); ?>" data-pt="<?php echo esc_attr($atts['post_type']); ?>">
    ...
    (function(){
      ...
      function scrollToRankingTop(){
        try{
          const anchor = document.getElementById('cdbRTop');
      ...
      function render(){
        ...
        pager.querySelectorAll('button[data-p]').forEach(btn=> btn.addEventListener('click', ()=> { const p=parseInt(btn.dataset.p,10); fetchPage(p); scrollToRankingTop(); }));
      }
      ...
      ['input','change'].forEach(ev=>{
        q.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
        sort.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
        corp.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
        addr.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
        flag.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
        min.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
        max.addEventListener(ev, ()=> { fetchPage(1); scrollToRankingTop(); });
      });
